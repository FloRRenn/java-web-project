...
public class PaymentController {
	...
	@Operation(
			hidden = true,
			summary = "Create Payment Service (User is required)",
			responses = {
					@ApiResponse( responseCode = "200", description = "Create payment successfully.",
							content = @Content( mediaType = "application/json", schema = @Schema(implementation = PaymentResponse.class))),
					@ApiResponse( responseCode = "400", description = "This ticket have been already paid or canceled before..",
							content = @Content( mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class))),
					@ApiResponse( responseCode = "404", description = "Ticket' ID is not found.",
							content = @Content( mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class)))
			},
			parameters = {
	...
	public ResponseEntity<?> createPayment(Principal principal, @Valid @RequestBody PaymentRequest request, HttpServletRequest servletRequest) {
		return ResponseEntity.ok().body(paymentSER.create(principal.getName(), request, "13.160.92.202")); //servletRequest.getRemoteAddr()
	}
	
	@GetMapping("/{id}")
	@Operation(
			summary = "Id Payment (User is required)",
			responses = {
					@ApiResponse( responseCode = "200", description = "Create ID payment successfully.",
							content = @Content( mediaType = "application/json", schema = @Schema(implementation = PaymentResponse.class))),					
	...
	@Operation(
			summary = "Id Payment (User is required)",
			responses = {
					@ApiResponse( responseCode = "200", description = "Verify ID payment successfully.",
							content = @Content( mediaType = "application/json", schema = @Schema(implementation = MyApiResponse.class))),					
					@ApiResponse( responseCode = "404", description = "Payment ID is not found.",
							content = @Content( mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class)))
			},
			parameters = {
					@Parameter( name = "Authorication", in = ParameterIn.HEADER,
	...
	@GetMapping("/getall")
	@Operation(
			summary = "Get All  Payment Of User (User is required)",
			responses = {
					@ApiResponse( responseCode = "200", description = "Get all  payment of user successfully.",
							content = @Content( mediaType = "application/json", schema = @Schema(implementation = MyApiResponse.class))),					
